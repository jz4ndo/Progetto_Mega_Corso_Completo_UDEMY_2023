...
public class AggiungiRecensione extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// se non � un admin, mostragli un messaggio di errore
		...
		if(utente.getTipo() != TipoUtente.ADMIN) {
			...
			// 2) l'utente vuole visualizzare una recensione gi� inserita -> Modifica != null
			if (request.getParameter("Salva") != null) {
				...
				// l'utente vuole visualizzare una recensione gi� inserita (e forse anche modificare)
				...
				// l'utente vuole eliminare una recensione gi� inserita
				// prendi l'id della recensione
				String temp_id = (String) request.getParameter("id");
				if (temp_id!=null) {
					Integer id = Integer.parseInt(request.getParameter("id"));
					// controlla se esiste e se l'utente � il suo autore
					// se si cancella
					Recensione recensione;
					try {
						recensione = RecensioneQuery.get_recensione(id);
